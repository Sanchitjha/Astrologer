...
export default function DecorCard({ slug, title, icon, intro, photo }) {
  return (
    <article className="glass-card rounded-2xl overflow-hidden flex flex-col group">
      ...
      <div className="p-4 flex flex-col flex-1 gap-2">
        <h3 className="text-[#2A1408]/85 text-sm font-semibold leading-snug">{title}</h3>
        <p className="text-[#2A1408]/50 text-xs leading-relaxed line-clamp-2 flex-1">{intro}</p>

        <Link
          href={`/spiritual-decor/${slug}`}
