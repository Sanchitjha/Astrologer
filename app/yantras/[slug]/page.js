      ...
      <main className="bg-[#050005] py-16 px-4">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <div className="glass-card rounded-3xl p-8 md:p-12 grid md:grid-cols-2 gap-10 items-center">
            ...
            >
              <span className="text-8xl md:text-9xl text-[#D4AF37]/50 float-anim select-none"
            ...
            <div className="space-y-5">
              ...
              <p className="text-white/55 text-sm leading-relaxed">{yantra.desc}</p>
                ...
                <div className="space-y-1.5">
                  {yantra.benefits.map((b) => (
                    <div key={b} className="flex items-center gap-2 text-sm text-white/60">
                      <span className="text-[#D4AF37] text-xs">✦</span> {b}
                    </div>
                  ))}
                </div>
              )}
          ...
          {related.length > 0 && (
            <div>
              <h2 className="text-white/60 text-sm font-semibold mb-5 text-center section-label">
                Related Yantras
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {related.map((r) => (
                  ...
                  >
                    <span className="text-3xl text-[#D4AF37]/50 block"
                          style={{ fontFamily: 'var(--font-devanagari)' }}>ॐ</span>
                    <p className="text-white/75 text-xs leading-snug line-clamp-2">{r.name}</p>
                    <p className="text-[#FFD700] font-bold text-sm">{formatINR(r.salePrice)}</p>
                  </Link>
                ))}
              </div>
            </div>
